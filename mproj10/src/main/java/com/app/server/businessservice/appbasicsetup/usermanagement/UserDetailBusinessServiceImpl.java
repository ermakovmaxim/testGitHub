...
public class UserDetailBusinessServiceImpl implements UserDetailBusinessService {
    ...
    public void update(List<UserDetail> entity) throws Exception {
        for (UserDetail _userdetail : entity) {
            if (_userdetail.isHardDelete()) {
                userDetailRepository.delete(_userdetail.getUserId());
            } else {
                userDetailRepository.deletePassRecovery(_userdetail.getDeletedPassRecoveryList());
                userDetailRepository.update(_userdetail);
            }
        }
    }
}
