...
@SourceCodeAuthorClass(createdBy = "john.doe", updatedBy = "john.doe", versionNumber = "3", comments = "Repository for PhoneCommunication Transaction table", complexity = Complexity.MEDIUM)
public class PhoneCommunicationRepositoryImpl extends CommonUtilRepositoryImpl implements PhoneCommunicationRepository<PhoneCommunication> {
    ...
    public List<CommonEntityInterface> findPageWiseData(String jpqlQuery, int pageSize, int pageNo) throws Exception {
        ...
        Log.out.println("ORGCM324100200", runtimeLogInfoHelper.getRequestHeaderBean(), "PhoneCommunicationRepositoryImpl", "findPageWiseData", "Total Records Fetched = " + listOfCountry.size());
