...
@SourceCodeAuthorClass(createdBy = "john.doe", updatedBy = "john.doe", versionNumber = "3", comments = "Repository for SocialCategory Master table Entity", complexity = Complexity.LOW)
public class SocialCategoryRepositoryImpl extends CommonUtilRepositoryImpl implements SocialCategoryRepository<SocialCategory> {
    ...
    public List<CommonEntityInterface> findPageWiseData(String jpqlQuery, int pageSize, int pageNo) throws Exception {
        ...
        Log.out.println("ORGCM324100200", runtimeLogInfoHelper.getRequestHeaderBean(), "SocialCategoryRepositoryImpl", "findPageWiseData", "Total Records Fetched = " + listOfCountry.size());
