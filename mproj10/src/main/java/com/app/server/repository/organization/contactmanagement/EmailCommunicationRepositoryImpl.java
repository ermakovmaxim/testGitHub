...
@SourceCodeAuthorClass(createdBy = "john.doe", updatedBy = "john.doe", versionNumber = "3", comments = "Repository for EmailCommunication Transaction table", complexity = Complexity.MEDIUM)
public class EmailCommunicationRepositoryImpl extends CommonUtilRepositoryImpl implements EmailCommunicationRepository<EmailCommunication> {
    ...
    public List<CommonEntityInterface> findPageWiseData(String jpqlQuery, int pageSize, int pageNo) throws Exception {
        ...
        Log.out.println("ORGCM324100200", runtimeLogInfoHelper.getRequestHeaderBean(), "EmailCommunicationRepositoryImpl", "findPageWiseData", "Total Records Fetched = " + listOfCountry.size());
