...
@SourceCodeAuthorClass(createdBy = "john.doe", updatedBy = "john.doe", versionNumber = "2", comments = "Repository for PhoneCategory Master table Entity", complexity = Complexity.LOW)
public class PhoneCategoryRepositoryImpl extends CommonUtilRepositoryImpl implements PhoneCategoryRepository<PhoneCategory> {
    ...
    public List<CommonEntityInterface> findPageWiseData(String jpqlQuery, int pageSize, int pageNo) throws Exception {
        ...
        Log.out.println("ORGCM324100200", runtimeLogInfoHelper.getRequestHeaderBean(), "PhoneCategoryRepositoryImpl", "findPageWiseData", "Total Records Fetched = " + listOfCountry.size());
